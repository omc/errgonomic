...
- `errgonomic_serialize_none :omit` drops the keys a record has no value for instead of writing them as `null`, declared on a model or on a base class above it, and scoped to named readers with `only:` or `except:`. The nearest declaration wins and replaces whatever it inherits; `:null` is the default and needs no declaration, and any other mode raises `ArgumentError`
