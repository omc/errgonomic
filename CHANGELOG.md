...
- Every query bind unwraps an Option before the column type sees it, so `find`, `find_by` and `exists?` take one on any column: a `json` column, whose type encodes the value it is handed without calling `super`, and an application's own `ActiveModel::Type::Value` subclass, which is written the same way
