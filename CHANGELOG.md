...
- Every query bind unwraps an Option before the column type sees it, so `find`, `find_by` and `exists?` take one on any column: a `json` column, whose type encodes the value it is handed without calling `super`, and an application's own `ActiveModel::Type::Value` subclass, which is written the same way. This is also what makes `find_by` usable against an encrypted attribute, whose type serializes through the underlying type and then calls `to_s`
- An `attribute :col, type, default: Some(v)` declaration unwraps its default where it is written, so the stored default is a plain value whatever the type is, as an assigned one is. A `Proc` default is left alone
